...
func main() {
	//if len(os.Args) < 2 {
	//	fmt.Println("need config file.eg: nil config.yaml")
	//	return
	//}
	...
	//加载email配置
	if err := email.InitEmail(settings.Conf.EmailConfig); err != nil {
		fmt.Printf("init emial failed, err:%v\n", err)
		return
	}

	//加载oss配置
	if err := oss.Init(settings.Conf.OssConfig); err != nil {
